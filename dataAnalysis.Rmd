...
knitr::opts_chunk$set(echo=FALSE, warning = FALSE, message = FALSE, echo=FALSE, include=FALSE)
...
```{r figure 2, fig.width=6, fig.height=4, fig.cap="Figure 2: Temporal memory accuracy by trial direction. The diamonds represent the mean accuracy for each trial direction, and the black dots represent individual participants. The dotted line represents chance-level performance.", echo=FALSE}
...
```{r figure 3, fig.width=12, fig.height=8, fig.cap="Figure 3: Proportion of incorrect responses at retrieval by encoding location and order. The x-axis shows the encoding location (L = left, M = middle, R = right) and order. The legend shows actual order -> guessed order.", echo=FALSE}
# Visualize mistakes by order and position (across both blocks)
# Color vision deficiency safe colors
mistake_plot <- ggplot(mistake_group %>% filter(mistake_type != "correct"),
...
mistake_chi
```
Because participants made more mistakes in the right-to-left and nonlinear trials relative to left-to-right trials, we explored whether participants’ mistakes suggested mental organization of temporal order in terms of the mental timeline. For this analysis, we compared participants’ incorrect responses for items presented first and third in the triplets and on the left and right sides of the screen to see if participants were biased towards associating first images with the right side of the screen and third images with the left. Participants were significantly more likely to mistakenly recall an image presented third in the triplet as being presented first when the image was presented on the left side of the screen relative to when it was presented on the right side of the screen and more likely to mistakenly recall an image presented first in the triplet as presented third when the image was presented on the right side of the screen relative to when it was presented on the right side of the screen ($\chi^2$(`r mistake_chi$parameter`) = `r round(mistake_chi$statistic,2)`, $p$ `r format_p(mistake_chi$p.value)`) (See Figure 3). These results suggest that the spatial location of the items contributed to participants’ recall errors when they were incongruent with the mental timeline.

#2. Pupil Dilation at Encoding
```{r encoding pup dilation}
...
```{r plot trial dir x pupil dilation}
...
```{r figures 3 and 4, fig.width=14, fig.height=10, fig.cap="Relative pupil dilation over the first view of the encoding trial by trial direction. The top panel represents what is present on the screen during the time period. The areas shaded in purple represent the times at which the triplet images were displayed, and the area shaded in gray represents the period from which the values used for the analyses of encoding.", echo=FALSE}
# Pink-purple colors are when triplet images appear
ggplot(data = enc_plot_data,
       aes(x=new_time, y = pupil, colour=as.factor(trialDirection))) + 
  geom_line() + geom_point() + 
  annotate("text", x=1500, y=0.1 , label = "Fixation", size = 3) +
  annotate("rect", xmin=2000, xmax=3500, ymin=-Inf, ymax=Inf, alpha=0.2, fill="blueviolet") +
  annotate("text", x=2750, y=0.1 , label = "Triplet 1", size = 3) + 
  annotate("rect", xmin=4000, xmax=5500, ymin=-Inf, ymax=Inf, alpha=0.2, fill="blueviolet") +
  annotate("text", x=4750, y=0.1 , label = "Triplet 2", size = 3) + 
  annotate("rect", xmin=6000, xmax=7500, ymin=-Inf, ymax=Inf, alpha=0.2, fill="blueviolet") +
  annotate("text", x=6750, y=0.1 , label = "Triplet 3", size = 3) +
  annotate("text", x=8200, y=0.1 , label = "Mask", size = 3) +
...
```{r trial dir x pupil dilation model}
...
First, we explored whether participants’ pupil dilation during the encoding phase changed as a function of trial direction. To explore changes in pupil dilation in relation to trial direction, we analyzed the baseline-corrected pupil values during two phases of the encoding trial. First, we explored the period starting from 1000 ms after the onset of the first image and extending until half a second after the end of the third image, corresponding to a 5000 ms period. This 1000 ms delay was implemented because the cognitive pupil dilation response emerges slowly (Hoeks & Levelt, 1993; Mathôt & Vilotijević, 2023). For the same reason, we included half a second period after the offset of the third image to capture responses to the third image (see Figure 4). Because both mental fatigue and familiarity with the experiment can influence pupil dilation (e.g., due to developing memory strategies), we used block as a fixed effect in this model (model syntax: relative pupil dilation ~ trial direction + block + (1|subject)).

Relative to trials displayed from left-to-right, participants experienced significantly larger pupil dilation when trials were displayed from right-to-left ($\beta$ = `r round(rel_pup_dir_block@beta[3],2)`,  $p$ `r format_p(ref_rel_pup_dir$coefficients[3,5])`) or nonlinearly ($\beta$ = `r round(rel_pup_dir_block@beta[2],2)`,  $p$ `r format_p(ref_rel_pup_dir$coefficients[2,5])`). There was a main effect of the block, such that participants had higher pupil dilation during the second block ($\beta$ = `r round(rel_pup_dir_block@beta[4],3)`,  $p$ `r format_p(ref_rel_pup_dir$coefficients[4,5])`). Because there was a main effect of block, we included block as a fixed effect in all future models.

We then analyzed mean and maximum pupil dilation values as a function of trial direction. Mean or maximum pupil dilation values did not vary as a function of trial direction (all $\beta$s < |`r round(mean_pup_dir@beta[3],2)`|, all $p$s > `r format_p(ref_mean_pup_dir$coefficients[3,5])`). This pattern of results suggests that differences in pupil dilation for different trial directions may in part be due to different dilation patterns over the course of multiple seconds, rather than an overall increase in pupil dilation.``

## Pupil dilation at encoding by retrieval accuracy
```{r encoding dilation with accuracy}
...
```{r stats for accyracy by trial direction and pupil dilation}
...
```{r trial dir x pupil dilation during mask}
# Summarize mean and maximum pupil dilation values per trial
...
Next, we explored the participants’ pupil dilation after viewing the triplets, while the mask image was on the screen. For this analysis, we included pupil values for the time period corresponding to a 2000 ms window after the third triplet image disappeared (See Figure 4). Because pupil dilation carryover effects can last 2000-3000 ms after stimulus onset, and because pupil dilation response to internal cognitive processes can take longer to arise relative to the initial visual response, this additional exploration was to assess whether these two time periods, one of actively viewing the images and committing the triplet to memory and the other of maintaining the triplet in memory, may reveal different patterns. As in previous analyses, because both mental fatigue and memory strategies can influence pupil dilation and retrieval accuracy, we used block as a fixed effect in these models (model syntax: relative pupil dilation ~ trial direction + block + (1|subject).

Relative to trials displayed from left to right, participants experienced significantly less pupil dilation when trials were displayed nonlinearly ($\beta$ = `r round(maskpup_dir@beta[2],2)`,  $p$ `r format_p(ref_maskpup_dir$coefficients[2,5])`) but not from trials displayed from right-to-left ($\beta$ = `r round(maskpup_dir@beta[3],3)`, $p$ `r format_p(ref_maskpup_dir$coefficients[3,5])`) (see Figure 5). There was a main effect of block, however, it was in the opposite pattern from prior analyses such that participants had overall lower pupil dilation during the second block compared to the first block ($\beta$ = `r round(maskpup_dir@beta[4],2)`,  $p$ `r format_p(ref_maskpup_dir$coefficients[4,5])`). We then analyzed mean and maximum pupil dilation values during the maintenance period as a function of trial direction. Trial direction did not influence mean or maximum pupil values during the two seconds following the initial triplet presentations(all $\beta$s < |`r round(maskpup_mean@beta[2],2)`|, all $p$s > `r format_p(ref_maskpup_mean$coefficients[2,5])`). This pattern of results suggests that differences in pupil dilation for different trial directions during the period while the triplet is being actively maintained in memory may in part be due to differences in sustained dilation.

### Recall Accuracy by Pupil Dilation at Mask
```{r encoding dilation with accuracy mask}
...
```{r stats: trial direction and pupil dilation}
# Make sure all data types are as expected before running stats
...
Next, we explored whether mean and maximum pupil dilation during either phase (while viewing the images or after viewing the images) of the encoding trials predicted accuracy at the retrieval phase. In these models, we also included encoding order (first, second, or third) as a fixed effect, to control for the possibility that item order within the triplet may influence memory accuracy (model syntax for mean dilation: recall accuracy ~ mean pupil dilation * trial direction + encoding order + block + (1| subject); model syntax for maximum dilation: recall accuracy ~ maximum pupil dilation * trial direction + encoding order + block + (1| subject)). 

In all four models, there was a main effect of trial type, such that, relative to trials presented from left to right, participants were less accurate for trials in which the triplets were presented from right to left (all $\beta$s < `r round(ref_acc_max_dilation$coefficients[4,1],2)`, all $p$s < `r format_p(ref_acc_max_dilation$coefficients[4,4])`) or nonlinearly (all $\beta$s < `r round(ref_acc_mean_dilation$coefficients[3,1],2)`, all $p$s < `r format_p(ref_acc_mean_dilation$coefficients[3,4])`). There was a main effect of encoding order in all models, such that the images presented third in the triplet were less likely to be accurately remembered relative to those presented first in the triplet (all $\beta$s < `r round(acc_by_max_dilation@beta[6],2)`, all $p$s < `r format_p(ref_acc_max_dilation$coefficients[6,4])`). However, neither mean nor maximum pupil dilation values at encoding predict accuracy during retrieval in either phase (all $\beta$s >`r round(acc_by_mean_dilation@beta[2],2)`, all $p$s < `r format_p(ref_acc_mean_dilation$coefficients[2,4])`) This pattern of results suggests though the presentation order and position within a triplet predicted memory accuracy, pupil dilation during either encoding or maintenance did not predict memory accuracy. 

Because trial directions incongruent with the mental timeline yielded both higher pupil dilation and lower accuracy, we ran separate analyses for each trial direction (model syntax: recall accuracy ~ mean pupil dilation + encoding order + block + (1| subject)). We found that for trials that were presented from right to left, lower mean and maximum pupil dilation values during both encoding and maintenance predicted correct answers at retrieval(all $\beta$s <`r round(acc_RL_max_dilation@beta[2],2)`, all $p$s < `r format_p(ref_acc_RL_max_dilation$coefficients[2,4])`). For trials presented from left to right, mean or maximum dilation during either phase did not predict correct answers at retrieval (all $\beta$s = `r round(acc_LR_mean_dilation@beta[2],2)`, all $p$s > `r format_p(ref_acc_LR_mean_dilation$coefficients[2,4])`). For trials presented nonlinearly,  mean or maximum dilation values during either phase did not predict accuracy (all $\beta$s > `r round(acc_NL_mean_dilation@beta[2],2)`, all $p$s > `r format_p(ref_acc_NL_mean_dilation$coefficients[2,4])`). All models ran for the nonlinear trials also had a main effect of encoding order, such that participants were significantly less likely to remember items that were presented second (all $\beta$s < `r round(mask_acc_NL_max_dilation@beta[3],2)`|, all $p$s < `r format_p(ref_mask_acc_NL_max_dilation$coefficients[3,4])`) or third (all $\beta$s < `r round(acc_NL_max_dilation@beta[4],2)`, all $p$s > `r format_p(ref_acc_NL_max_dilation$coefficients[4,4])`) in the nonlinear trials. For trials presented from left to right or right to left, there was no main effect of encoding order (all $\beta$s < |`r round(acc_RL_mean_dilation@beta[3],2)`|, all $p$s > `r format_p(ref_acc_RL_mean_dilation$coefficients[3,4])`). These results suggests that accuracy at retrieval only predicted pupil dilation at encoding for the right-to-left trials. Further, it is likely that the main effect of encoding order in the main model was due to increased difficulty in remembering items presented later in the trial for nonlinear trials.

#3. Gaze location at retrieval
## Rolling averages of gaze location:
```{r gaze patterns by encoding position}
...
```{r corr gaze location by encoding order and position}
...
Our second main question of interest was whether participants’ gaze movements at retrieval suggest that they spontaneously organized the temporal order of triplets in terms of the mental timeline. To answer this question, we analyzed the 1000 ms time window between the offset of the retrieval image and the onset of the answer options. We first examined participants’ gaze positions to assess whether the order or location of the retrieval image during encoding influenced gaze movements during successful retrieval trials. We then compared whether gaze patterns differed between trials with accurate and inaccurate retrieval. For both analyses, we calculated rolling averages of the x and y-coordinates of participants’ gaze using a five-point moving window via the zoo package (Zeileis & Grothendieck, 2005), which averaged five consecutive points over the target duration. We used these rolling averages to smooth fluctuations in the raw data and extract broader, systematic shifts in gaze movement. All gaze analyses were also conducted using raw gaze data and by calculating displacement between gaze positions at the start and end of the 1000 ms interval. Because all methods yielded similar patterns, only the rolling average analyses are reported here.

***Encoding Order***
We explored participants’ gaze movement for correct trials as a function of the temporal order in which the retrieval item appeared during the encoding phase (model syntax: rolling gaze x-coordinate ~ encoding locationorder + (1| subject_id).)). We found that the x-coordinate location of the gaze position was influenced by encoding locationorder, such that relative to images presented first, participants moved their gaze rightward when recalling retrieval images presented in looked more toward the right side of the middle during encoding (β = 5.49, p < .001) and screen for images presented on the right during encodingsecond ($\beta$ = `r round(ret_meanx_order@beta[2],2)`,  $p$ `r format_p(ref_ret_meanx_order$coefficients[2,5])`). However, there was no difference for gaze location for images presented third ($\beta$ = `r round(ret_meanx_order@beta[3],2)`,  $p$ `r format_p(ref_ret_meanx_order$coefficients[3,5])`) (see Figure 6). These results suggest that participants did not reconstruct the triplets in terms of the mental timeline such that they represented items presented first, second, and third in the triplet in the left, middle, and right of space. 
```{r average gaze locations for plots}
# Using a "pre-decision" phase, for which for one second participants see a blank screen
# This is the same df used to calculate the 'rolling mean' locations of the gaze positions, this is an overall mean of the same period instead of rolling means in those timebins. 
# For encoding order
mean_gaze_order <- preDecision %>%
  filter(!is.na(is_correct)) %>% 
  group_by(encoding_order, is_correct) %>%
  summarise(
    mean_gaze_x   = mean(LEFT_GAZE_X, na.rm = TRUE),
    gaze_x_sd     = sd(LEFT_GAZE_X, na.rm = TRUE),
    gaze_x_length = sum(!is.na(LEFT_GAZE_X)),
    gaze_x_sem    = gaze_x_sd / sqrt(gaze_x_length),
    upper_CI      = ci.high(LEFT_GAZE_X),
    lower_CI      = ci.low(LEFT_GAZE_X),
    .groups = "drop"
  ) %>%
  mutate(
    across(c(encoding_order, is_correct), as.factor),
    encoding_order = factor(
      encoding_order, levels = c("first", "second", "third")))
# For encoding location
mean_gaze_location <- preDecision %>%
  filter(!is.na(is_correct)) %>% 
  group_by(encoding_position, is_correct) %>%
  summarise(
    mean_gaze_x   = mean(LEFT_GAZE_X, na.rm = TRUE),
    gaze_x_sd     = sd(LEFT_GAZE_X, na.rm = TRUE),
    gaze_x_length = sum(!is.na(LEFT_GAZE_X)),
  ...
  mutate(
    across(c(encoding_position, is_correct), as.factor),
    encoding_position = factor(
      encoding_position, levels = c("L", "M", "R")))

summary(preDecision)
```

```{r figure 6, fig.width=12, fig.height=6, fig.cap="Figure 6. Gaze location on the x-axis by encoding order, during the blank screen period at retrieval trials. The points represent the mean gaze position. The dashed line represents the median gaze location for the analyzed period. The left side of the plot represents more leftward gaze locations, and the right side represents more rightward locations.", echo=FALSE}
# Encoding order gaze location
ggplot(mean_gaze_order,
       aes(x = mean_gaze_x,
           y = encoding_order,
           color = encoding_order)) +
  geom_point(size = 5) +
  geom_errorbarh(aes(xmin = lower_CI, xmax = upper_CI), width = 0.15) +
  facet_wrap(~ is_correct,
    labeller = as_labeller(c('0' = "Incorrect", '1' = "Correct"))) +
  scale_color_manual(values = c("#d95f02", "#1b9e77", "#7570b3"), 
                     name = "Encoding Order",
                     labels = c("First", "Second", "Third")) +
  geom_vline(xintercept=653, linetype="dashed", color = "red", alpha = 0.5) +
  annotate("text", x=641, y=3.5, label = "Gaze direction more left", size = 2.5) + 
  annotate("text", x=658, y=3.5, label = "Gaze direction more right", size = 2.5) +
  scale_y_discrete(limits = c("third", "second", "first"),
                   labels = c("Third", "Second", "First")) +
  scale_x_continuous(limits = c(639,660)) +
  labs(
    title = "Mean Gaze X Order During Retrieval",
    x = "Mean Gaze X Coordinate",
    y = "Encoding Order") +
  theme_Publication()
```

***Encoding Location***
We explored participants’ gaze movement for correct trials as a function of the location in which the retrieval item appeared during the encoding phase (model syntax: rolling gaze x-coordinate ~ encoding location + (1| subject). We found that the x-coordinate location of the gaze position during retrieval was influenced by encoding location, such that relative to images presented on the left, participants moved their gaze rightward when recalling images initially presented in the middle ($\beta$ = `r round(ret_meanx_location@beta[2],2)`,  $p$ `r format_p(ref_ret_meanx_location$coefficients[2,5])`) and on the right ($\beta$ = `r round(ret_meanx_location@beta[3],2)`,  $p$ `r format_p(ref_ret_meanx_location$coefficients[3,5])`) (see Figure 7). This pattern of results suggests that during retrieval, participants referenced the locations in which they initially saw the images during the encoding phase by moving their gaze toward those locations.

```{r figure 7, fig.width=11, fig.height=6, fig.cap="Figure  7. Gaze location on the x-axis by encoding location, during the blank screen period at retrieval trials. The points represent the mean gaze position. The dashed line represents the median gaze location for the analyzed period. The left side of the plot represents more leftward gaze locations, and the right side represents more rightward locations.", echo=FALSE}
# Encoding position gaze location
ggplot(mean_gaze_location,
       aes(x = mean_gaze_x,
           y = encoding_position,
           color = encoding_position)) +
  geom_point(size = 5) +
  geom_errorbarh(aes(xmin = lower_CI, xmax = upper_CI), width = 0.15) +
  facet_wrap(~ is_correct,
    labeller = as_labeller(c('0' = "Incorrect", '1' = "Correct"))) +
  scale_color_manual(values = c("#d95f02", "#1b9e77", "#7570b3"), 
                     name = "Encoding Location",
                     labels = c("Left", "Middle", "Right")) +
  geom_vline(xintercept=653, linetype="dashed", color = "red", alpha = 0.5) +
  annotate("text", x=641, y=3.5, label = "Gaze more left", size = 2.5) + 
  annotate("text", x=658, y=3.5, label = "Gaze more right", size = 2.5) +
  scale_y_discrete(limits = c("R", "M", "L"),
                   labels = c("Right", "Middle", "Left")) +
  scale_x_continuous(limits = c(639,660)) +
  labs(
    title = "Mean Gaze X Location During Retrieval",
    x = "Mean Gaze X Coordinate",
    y = "Encoding Location") +
  theme_Publication()
```
...
Next, we explored whether the correct retrieval of the temporal order of the retrieval item was related to participants’ eye movements. We first explored whether gaze patterns differed between correct and incorrect trials as a function of encoding order (model syntax: rolling gaze x-coordinate ~ encoding order * correct answer + (1| subject)). There was an interaction between accuracy and encoding order, such that participants were more likely to move their gaze rightward for retrieval images that were presented second and answered correctly relative to those recalled incorrectly ($\beta$ = `r round(acc_meanx_order@beta[5],2)`,  $p$ `r format_p(ref_acc_meanx_order$coefficients[5,5])`) but not there was no difference in gaze movements for retrieval images that were presented third and answered correctly ($\beta$ = `r round(acc_meanx_order@beta[6],2)`,  $p$ `r format_p(ref_acc_meanx_order$coefficients[6,5])`). This pattern of results suggests that for correctly recalled items, participants did not consistently show evidence of reconstructing temporal order in terms of the mental timeline. 

Finally, we explored whether gaze patterns differed between correct and incorrect trials, as a function of encoding location (model syntax: rolling gaze x ~ encoding location * correct response + (1| subject_id). There was also an interaction between accuracy and encoding location, such that participants were more likely to move their gaze rightward for retrieval images that were presented in the middle and answered correctly ($\beta$ = `r round(acc_meanx_location@beta[5],2)`,  $p$ `r format_p(ref_acc_meanx_location$coefficients[5,5])`) and for retrieval images that were presented on the right and answered correctly ($\beta$ = `r round(acc_meanx_location@beta[6],2)`,  $p$ `r format_p(ref_acc_meanx_location$coefficients[6,5])`). These results suggest that differences in gaze patterns for correctly and incorrectly recalled items may be in part due to participants’ spontaneous referencing of the locations where the items were presented during the encoding phase.

# 4. Exploratory Analyses: Internal Representations, Strategy, and Recall
#Clean and reshape data
```{r data reshaping and cleaning}
...
```
#Strategy use data
```{r self resported strategy use}
#for knitting
strategy_data <- read.csv(here("OSF/Analysis/TG_Strategy.csv"))
...
In addition to our main questions of interest, we analyzed whether participants’ response patterns interacted with their self-reported strategy use and their tendencies for using different modes of thinking (e.g., having vivid mental imagery) (Roebuck & Lupyan, 2020). Twenty-six participants indicated using a strategy during the task. Most participants reported using a strategy either right at the beginning of the task (*n* = `r strategy_summary$beginning`) or somtime during the first encoding block (*n* = `r strategy_summary$before_break`). Over half of the participants reported using only a verbal-only strategy (*n* = `r strategy_summary$n_verbal`) (e.g., *"I just repeated the order in my head (ex: pan-right, lady bug-middle, tree-left")*). Many participants reported using a mix of verbal and spatial strategies (*n* = `r strategy_summary$n_both`)  (e.g., *“I tried to say the words in my head and repeat them so I could remember the order, as well as following with my eyes the original order that I saw*”). Two participants reported using a spatial-only strategy (e.g., “*Going from left to right or right to left each time even if the first image appeared on the middle of the screen. That way I could orient the object order in my mind no matter when and where objects popped up on the screen*”) and two participants reported strategies that did not fit in any category (e.g., "*Remember the first and last one*").

## Strategy X Recall
```{r strategy and recall}
...
```{r stats for strat x accuracy x trial direction}
...
We first explored whether participants’ self-reported strategy use interacted with their accuracy for different trial directions. Because few participants used a spatial-only or vague strategy, we explored the effects of using a verbal-only strategy versus a mix of verbal and spatial strategies (model syntax: correct answer ~ trial direction * strategy type + (1| subject).  Using a verbal-only strategy trended towards predicting lower accuracy for trials presented from right-to-left ($\beta$ = `r round(acc_strategy_dir@beta[6],2)`, $p$ `r format_p(ref_acc_strategy_dir$coefficients[6,4])`) and predicted lower accuracy for trials presented nonlinearly ($\beta$ = `r round(acc_strategy_dir@beta[5],2)`, $p$ `r format_p(ref_acc_strategy_dir$coefficients[5,4])`) relative to using a mix of verbal and spatial strategies. This pattern of results suggests that participants who used a verbal-only strategy experienced a stronger temporal memory disadvantage when items in the triplet were presented incongruent with the mental timeline relative to those who used a mix of verbal and spatial strategies. 

#IRQ factors
## Are IRQ scores correlated?
```{r visual and verbal IRQ scores correlation}
...
Next, we explored whether participants’ Internal Representations Questionnaire factor scores predicted their strategy type (model syntax: strategy type ~ internal verbalization scores + visual imagery scores + representational manipulation scores). Similar to the previous analysis, we explored how the factors scores relate to using verbal-only strategy versus a mix of verbal and spatial strategies. Overall, participants who had higher internal verbalization scores were more likely to report using a verbal only strategy relative to a mix of verbal and spatial strategies ($\beta$ = `r round(ref_IRQ_strat$coefficients[2,1],2)`, $p$ `r format_p(ref_IRQ_strat$coefficients[2,4])`). The visual imagery and representational manipulation factors did not predict strategy use (all $\beta$s < `r round(ref_IRQ_strat$coefficients[3,1],2)`, all $p$s > `r format_p(ref_IRQ_strat$coefficients[3,4])`). However, there was a significant positive correlation between the visual imagery and representational manipulation scores ($\rho$(30) =`r round(irq_cor_results$r["visual_c", "mental_c"],2)`, $p$ `r format_p(irq_cor_results$p["visual_c", "mental_c"])`). 
...
```{r stats for IRQ x accuracy x trial direction}
# 1. Do IRQ factor scores predict the interaction between accuracy and trial direction?
IRQ_dir_acc <- glmer(is_correct ~ originTrialDir * verbal_c + 
                       originTrialDir * visual_c + 
                       originTrialDir * mental_c + (1|Participant_ID),
                                family = binomial(link = "logit"), 
                     control = glmerControl(optimizer="bobyqa"),
                     data = recall_IRQ_combined)
ref_IRQ_dir_acc <- summary(IRQ_dir_acc)
```
...
Next, we explored whether the temporal memory benefit for left-to-right trials differed based on Internal Representations Questionnaire factor scores. We ran a model We ran a model predicting correct responses, in which the encoding trial direction interacted with each factor ( model syntax: correct response ~ trial direction * internal verbalization +  trial direction * visual imagery +  trial direction * representational manipulation + (1|subject)).

Internal verbalization scores did not predict overall memory accuracy ($\beta$ = `r round(IRQ_dir_acc@beta[4],2)`, $p$ `r format_p(ref_IRQ_dir_acc$coefficients[4,4])`). There was an interaction between internal verbalization scores and accuracy, such that participants with higher internal verbalization scores showed a greater decrease in accuracy on nonlinear trials ($\beta$ = `r round(IRQ_dir_acc@beta[7],2)`, $p$ `r format_p(ref_IRQ_dir_acc$coefficients[7,4])`) relative to those with lower scores.

There was a main effect of visual imagery scores on participants’ memory accuracy, such that participants who reported higher visual imagery scores also had lower memory accuracy relative to those with lower scores ($\beta$ = `r round(IRQ_dir_acc@beta[5],2)`, $p$ = `r format_p(ref_IRQ_dir_acc$coefficients[5,4])`). However, visual imagery scores did not interact with accuracy for any particular trial direction (all $\beta$s < `r round(IRQ_dir_acc@beta[9],2)`,all $p$s > `r format_p(ref_IRQ_dir_acc$coefficients[9,4])`)

Representational manipulation scores did not predict overall memory accuracy or accuracy for any trial direction and were not included in the subsequent analyses (all $\beta$s < |`r round(IRQ_dir_acc@beta[6],2)`|,all $p$s > `r format_p(ref_IRQ_dir_acc$coefficients[6,4])`). These results suggests that participants’ higher tendencies for verbalizing thoughts predicted a sharper decrease in temporal memory accuracy for nonlinear trials, and higher tendencies for visually representing thoughts predicted lower memory accuracy for all trial types. Overall, these results suggest that the benefit from left-to-right trials may differ based on people’s tendencies for internally representing information.

Because Internal Representations Questionnaire factor scores predicted temporal memory accuracy, we further probed the relation between these factors and the use of the mental timeline during memory retrieval. We explored participants’ proportion of incorrect responses for images coming from congruent (left-to-right) and incongruent (right-to-left, nonlinear) encoding trials and calculated the difference between their proportion of incorrect responses for congruent and incongruent trials. We then explored whether Internal Representations Questionnaire scores predicted the proportion of incorrect responses in congruent and incongruent trials (model syntax: difference in the proportion of mistakes ~ internal verbalization scores + visual imagery scores). This analysis confirmed that participants with higher internal verbalization scores were overall more likely to make more errors in the incongruent trials relative to congruent trials ($\beta$ = `r round(ref_mistakes_prop$coefficients[2,1],2)`, $p$ `r format_p(ref_mistakes_prop$coefficients[2,4])`); however, visual imagery scores did not predict the proportion of errors for congruent and incongruent trials ($\beta$ = `r round(ref_mistakes_prop$coefficients[3,1],2)`, $p$ `r format_p(ref_mistakes_prop$coefficients[3,4])`). These results suggest that for participants with higher tendencies for verbalizing their thoughts, the salience of spatial information may more dramatically increase errors for trials presented incongruent with the mental timeline.

5. Cluster based permutation tests
```{r prep data for cluster based permutation, eval=FALSE, include=FALSE}
# Shape the data
end_data_cbp <- enc_data_relative %>%
  filter(new_time < 9550)

# Set all the timebins available
all_times <- sort(unique(end_data_cbp$new_time))

# get all timepoints to 'match up' all the data (e.g., if there is no data after time 4800 in one trial, make it go up to 5000 still)
encoding_cbp_clean <- end_data_cbp %>%
  mutate(TRIAL_INDEX = as.factor(TRIAL_INDEX)) %>%
  group_by(RECORDING_SESSION_LABEL, trialDirection, TRIAL_INDEX, block) %>%
  # complete() expands the dataframe to include all values in 'all_times' per group
  complete(new_time = all_times) %>% 
  arrange(RECORDING_SESSION_LABEL, trialDirection, TRIAL_INDEX, block, new_time) %>%
  ungroup()

# Check how many NAs were introduced
summary(encoding_cbp_clean$rel_pup)
 
# get only the relevant columns
encoding_cbp_clean <- encoding_cbp_clean %>%
   select(c(RECORDING_SESSION_LABEL, trialDirection, TRIAL_INDEX, block, rel_pup, new_time))

# Quick sanity check: number of timebins per subject × condition (should be identical)
trial_check <- encoding_cbp_clean %>%
  group_by(RECORDING_SESSION_LABEL, TRIAL_INDEX) %>%
  summarize(n_bins = n(), .groups = "drop")

unique(trial_check$n_bins)

encoding_cbp_clean <- encoding_cbp_clean %>%
  mutate(across(c(RECORDING_SESSION_LABEL, trialDirection, block), as.factor))

# Now split the first view and mask parts again
enc_cbp_first <- encoding_cbp_clean %>%
  filter(new_time > 3050 & new_time < 8050)
  
enc_cbp_mask <- encoding_cbp_clean %>%
  filter(new_time > 7450)
```
# Because it takes time to run the full permutations, run the t-tests first (averaging each participants' pupil dilation per trial type)
```{r run t test permutations, eval=FALSE, include=FALSE}
# Far permutation for first view:
avg_firstview <- enc_cbp_first %>%
  group_by(RECORDING_SESSION_LABEL, trialDirection, block, new_time) %>%
  summarize(mean_pup = mean(rel_pup, na.rm = TRUE), .groups = "drop")

# Pivot to 'Wide' format 
wide_firstview <- avg_firstview %>%
  pivot_wider(names_from = new_time, values_from = mean_pup)

# Drop any rows with NAs (Permuco's clusterlm doesn't like NAs in the matrix)
wide_firstview <- na.omit(wide_firstview)

firstview_matrix <- as.matrix(wide_firstview[, -(1:3)])

# Run the permutation test
# We use trialDirection + block to keep your covariate in the model
firstview_t_results <- clusterlm(firstview_matrix ~ trialDirection + block, 
                          data = wide_firstview, 
                          np = 2000)

# Check the results
summary(firstview_t_results)

# Far permutation for mask view:
avg_mask <- enc_cbp_mask %>%
  group_by(RECORDING_SESSION_LABEL, trialDirection, block, new_time) %>%
  summarize(mean_pup = mean(rel_pup, na.rm = TRUE), .groups = "drop")
...
# Drop any rows with NAs (Permuco's clusterlm doesn't like NAs in the matrix)
wide_mask <- na.omit(wide_mask)

mask_matrix <- as.matrix(wide_mask[, -(1:3)])

# Run the permutation test
# We use trialDirection + block to keep your covariate in the model
mask_t_results <- clusterlm(mask_matrix ~ trialDirection + block, 
                          data = wide_mask, 
                          np = 2000)

# Check the results
summary(mask_t_results)

```
# Long perm run!
```{r run mixed effects model permutations, eval=FALSE, include=FALSE}
save.image("pre_perm_backup.RData")
# first view period
firstview_results <- clusterperm.lmer(rel_pup ~ trialDirection + block + (1|RECORDING_SESSION_LABEL), 
                            data = enc_cbp_first, 
                            series = "new_time", 
                            nperm = 1000)

print(summary(firstview_results))
print(plot(firstview_results))

# Get the time information back in the results data
# create the key from your original numeric data
time_key <- data.frame(
  index = as.character(1:length(unique(enc_cbp_first$new_time))),
  actual_ms = sort(unique(enc_cbp_first$new_time))
)

# fix the results data frame
firstview_results <- as.data.frame(firstview_results) %>%
  left_join(time_key, by = c("new_time" = "index")) %>%
  mutate(new_time = actual_ms) %>% # Overwrite the index with the real MS
  select(-actual_ms)               # Drop the extra column

firstview_cluster_table <- firstview_results %>%
  filter(!is.na(cluster) & p.cluster_mass < 0.05) %>%
  group_by(Factor, cluster) %>%
  summarize(
    start = min(new_time),
    end = max(new_time),
    duration = end - start,
    p_val = unique(p.cluster_mass),
    .groups = "drop"
  )


# mask period
mask_results <- clusterperm.lmer(rel_pup ~ trialDirection + block + (1|RECORDING_SESSION_LABEL), 
                            data = enc_cbp_mask, 
                            series = "new_time", 
                            nperm = 1000)

print(summary(mask_results))
print(plot(mask_results))

# Get the time information back in the results data
# create the key from your original numeric data
time_key <- data.frame(
  index = as.character(1:length(unique(enc_cbp_mask$new_time))),
  actual_ms = sort(unique(enc_cbp_mask$new_time))
)

# fix the results data frame
mask_results <- as.data.frame(mask_results) %>%
  left_join(time_key, by = c("new_time" = "index")) %>%
  mutate(new_time = actual_ms) %>% # Overwrite the index with the real MS
  select(-actual_ms)               # Drop the extra column

mask_cluster_table <- mask_results %>%
  filter(!is.na(cluster) & p.cluster_mass < 0.05) %>%
  group_by(Factor, cluster) %>%
  summarize(
    start = min(new_time),
...
# Combine cluster tables
# Add a label for the period and combine
combined_cluster_table <- bind_rows(
  firstview_cluster_table %>% mutate(period = "FirstView"),
  mask_cluster_table      %>% mutate(period = "Mask")
) %>%
  select(period, Factor, cluster, start, end, duration, p_val)
```


```{r visualize permutation results, eval=FALSE, include=FALSE}
# Quick visualization
# Make averaged df of pupil dilation to plot
pup_plot <- encoding_cbp_clean %>%
  group_by(new_time, trialDirection) %>%
  summarise(
    mean_pupil = mean(rel_pup, na.rm = TRUE),
    se_pupil = sd(rel_pup, na.rm = TRUE) / sqrt(n()),
    .groups = 'drop'
  )
...
direction_colors <- c("NL" = "#DDAA33", "RL" = "#BB5566", "LR" = "#004488")
# Use slightly different shades or a grey scale for the statistical bars
cluster_fills <- c("trialDirectionNL" = "#1b9e77", 
                   "trialDirectionRL" = "#d95f02", 
                   "block2" = "#666666")

ggplot(pup_plot, aes(x = new_time, y = mean_pupil, colour = as.factor(trialDirection))) + 
  geom_line() + 
  geom_point() + 
  annotate("text", x = 1500, y = 0.1, label = "Fixation", size = 3) +
  annotate("rect", xmin = 2000, xmax = 3500, ymin = 0.1, ymax = Inf, alpha = 0.1, fill = "blueviolet") +
  annotate("text", x = 2750, y = 0.1, label = "Triplet 1", size = 3) + 
  annotate("rect", xmin = 4000, xmax = 5500, ymin = 0.1, ymax = Inf, alpha = 0.1, fill = "blueviolet") +
  annotate("text", x = 4750, y = 0.1, label = "Triplet 2", size = 3) + 
  annotate("rect", xmin = 6000, xmax = 7500, ymin = 0.1, ymax = Inf, alpha = 0.1, fill = "blueviolet") +
  annotate("text", x = 6750, y = 0.1, label = "Triplet 3", size = 3) +
  annotate("text", x = 8200, y = 0.1, label = "Mask", size = 3) +
  # add lines to mark analyzed periods
  geom_vline(xintercept = 3000, linetype = "dashed", color = "gold3", alpha = 0.7) +
  geom_vline(xintercept = 8000, linetype = "dashed", color = "gold3", alpha = 0.7) +
  geom_vline(xintercept = 7500, linetype = "dashed", color = "firebrick", alpha = 0.7) +
  geom_vline(xintercept = 9500, linetype = "dashed", color = "firebrick", alpha = 0.7) +
    annotate("text", x = 3500, y = 0.1, label = "First view period", size = 5) + 
    annotate("text", x = 9000, y = 0.1, label = "Mask period", size = 5) + 
  geom_rect(data = combined_cluster_table, inherit.aes = FALSE,
            aes(xmin = start, xmax = end, 
                fill = Factor,
                ymin = case_when(
                  Factor == "trialDirectionNL" ~ 0.06,
                  Factor == "trialDirectionRL" ~ 0.04,
                  Factor == "block2"           ~ 0.03,
                  TRUE ~ -0.85
                ),
                ymax = case_when(
                  Factor == "trialDirectionNL" ~ 0.07,
                  Factor == "trialDirectionRL" ~ 0.06,
                  Factor == "block2"           ~ 0.05,
                  TRUE ~ -0.75
                )), alpha = 0.4) +
  scale_x_continuous(breaks = seq(1000, 9500, 1000), limits = c(1000, 9500)) +
  scale_color_manual(values = direction_colors, name = "Direction") +
  scale_fill_manual(values = cluster_fills, name = "Sig. Clusters (p < .05)") +
  coord_cartesian(ylim = c(-0.3, 0.1)) + 
  ylab("Relative pupil dilation (mm)") + 
  xlab("Time (ms)") +
  theme_Publication()
```
