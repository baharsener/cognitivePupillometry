...
```{r exclusion second pass, eval=FALSE, message=FALSE, include=FALSE}
...
```{r clean sample data, maessage=FALSE}
...
all_data <- all_data %>%
  mutate(across(c(RECORDING_SESSION_LABEL, trial_category, trialOrder, trialDirection, block),
                as.factor))
...
trial_counts <- all_data %>%
  group_by(trial_category) %>%
  summarize(unique_trials = n_distinct(paste(RECORDING_SESSION_LABEL, TRIAL_INDEX)))

# Write csv file for later use
write.csv(all_data, here("Analysis/csv_output/allData.csv"))

# Save missing data information too
write.csv(all_missing, here("Analysis/csv_output/missingData.csv"))
```

**Data Processing**
***Pupil data preprocessing***
Pupil data were processed and cleaned in R using the PupillometryR package (Forbes, 2020). Pupil data were first downsampled to 50 ms time bins, outlier data points (e.g., those corresponding to blinks) were filtered out using a median filter, and resulting gaps were linearly interpolated. Trials with 50% or more gaze data missing were removed from the analysis (*n* = `r missing_counts[1,2]` encoding, *n* = `r missing_counts[2,2]` retrieval).  For the encoding trials, only the ‘same’ trials were analyzed (80% of all encoding trials, *n* = 20 trials per block). We filtered out the encoding trials for which participants answered the attention check question incorrectly (*n* =  `r nrow(incorrect_method)` trials). In addition to filtering rules in our preregistered analysis plan, we removed retrieval trials with ‘impossible’ time stamps for the participants' responses were removed from analyses (e.g., participant’s response was recorded before answer options appeared on the screen) (*n* = `r nrow(removed_time_trials)` trials). Pupil values for the remaining trials (*n* = `r trial_counts[1,2]` encoding, *n* = `r `trial_counts[2,2]` retrieval) were converted from artificial units to mm using values obtained from a recording with an artificial pupil of a known size. 
...
# Make sure to filter excluded participants
filtered_file_list <- filtered_file_list[
  unlist(sapply(filtered_file_list, function(x) {
    Participant_ID <- str_extract(x, "\\d+")
    Participant_ID %in% participants$Participant_ID
...
all_recall_clean <- all_recall_clean %>%
  mutate(across(
    c(RECORDING_SESSION_LABEL, encoding_order, encoding_position, orderResponse, is_correct, block, originTrialDir),
    as.factor))
...
# How many trials?
print(n_distinct(retrieval_phase$RECORDING_SESSION_LABEL, 
                                   retrieval_phase$TRIAL_INDEX))
...
# Look at the order/position balancing of behavioral data
beh_trial_counts <- all_recall_clean %>%
  group_by(RECORDING_SESSION_LABEL, encoding_order, block) %>%
  summarize(trial_count = n(), .groups = 'drop',) %>%
  arrange(RECORDING_SESSION_LABEL, encoding_order) %>%
  filter(encoding_order != ".")
...
# Any participants with very few behavioral trials?
beh_retrieval_subj <- all_recall_clean %>%
  group_by(RECORDING_SESSION_LABEL, block) %>%
  summarize(trial_count = n(), .groups = 'drop')

# Check what the trial balance encoding trials look like after the exclusions
encTrials <- enc_data_relative %>%
  group_by(RECORDING_SESSION_LABEL, TRIAL_INDEX, block) %>%
  summarize(trialDirection = first(trialDirection), .groups = 'drop') %>%
  arrange(RECORDING_SESSION_LABEL, TRIAL_INDEX)
...
enc_missing_removed <- enc_subjTot %>%
  filter(exclude == TRUE) %>%
  summarise(sum(trial_count))
...
# Filter these participants or blocks from the encoding df: (currently only removing ones with more than 50% missing)
enc_data_relative <- enc_data_relative %>%
  anti_join(enc_subjTot %>% filter(exclude == TRUE), 
            by = c("RECORDING_SESSION_LABEL", "block"))

# Write the final dfs
write.csv(enc_data_relative, (here("Analysis/csv_output/enc_data_relative.csv")))
write.csv(retrieval_phase, (here("Analysis/csv_output/retrieval_phase.csv")))

# Get the final trial numbers
# Encoding trials
enc_trial_counts <- enc_data_relative %>%
  group_by(RECORDING_SESSION_LABEL, TRIAL_INDEX) %>%
  summarize(unique_trials = n_distinct(paste(RECORDING_SESSION_LABEL, TRIAL_INDEX)))

length(enc_trial_counts$unique_trials)

# Retrieval trials
ret_trial_counts <- retrieval_phase %>%
  group_by(RECORDING_SESSION_LABEL, TRIAL_INDEX) %>%
  summarize(unique_trials = n_distinct(paste(RECORDING_SESSION_LABEL, TRIAL_INDEX)))

length(ret_trial_counts$unique_trials)

# Memory accuracy trials
beh_trial_counts <- all_recall_clean %>%
  group_by(RECORDING_SESSION_LABEL, TRIAL_INDEX) %>%
  summarize(unique_trials = n_distinct(paste(RECORDING_SESSION_LABEL, TRIAL_INDEX)))

length(beh_trial_counts$unique_trials)
```
***Pupil dilation at encoding***
Our primary period of interest for measuring pupil dilation was the initial presentation of the triplet of images. For each trial and each participant, we calculated a baseline measure of pupil dilation that corresponded to the average pupil dilation during the final 100 ms of the fixation period before the first image in the triplet. Baseline pupil dilation values that were two standard deviations above or below the mean were considered outliers and excluded from the data (*n* = `r nrow(cut_enc)` trials). Two additional encoding trials were removed due to the experimenter pausing the study to recalibrate the eye tracker. In addition to the filtering rules in our preregistered analysis plan, we removed trials from blocks with 50% or fewer of encoding trials remaining. This removed one encoding block from three participants (*n* = 13 trials total), resulting in *n* = `r length(enc_trial_counts$unique_trials)` trials included in the pupil dilation analyses.

***Gaze location at retrieval***
Our primary period of interest for measuring gaze movements was the 1000 ms period in which participants saw a blank screen after viewing the retrieval image and before answer options appeared on the screen. In addition to filtering rules outlined in our preregistered analysis plan,  we filtered all gaze points that were projected to be outside the screen coordinates (5% of the points) because the eye-tracker projects gaze locations for off-screen looks. We also removed outlier gaze points that were two standard deviations away from the mean coordinates on a given trial (3% of the points), which removed three trials. This was to eliminate gaze positions captured while participants were making eye movements off the screen. Trials with response times for the recall questions that were two standard deviations above the mean for each participant were considered outliers and excluded from the data (*n* = `r length(ret_trial_counts$unique_trials)` trials). This resulted in one participant only having 50% of their retrieval trials remain for one block, so trials from that block were also removed (*n* = 10), and *n* = length(enc_trial_counts$unique_trials) trials were included in the gaze analyses. In our preregistered analysis plan, we planned to analyze the movement between participants’ final fixation point while the retrieval image was present, and their first fixation point after the retrieval image disappeared, using x-axis coordinates. However, because many participants did not have this data (e.g., due to looking off screen after seeing the image), we used raw gaze data to calculate gaze movements during the retrieval period to better account for variations in data, such as gaze drifts. 

***Behavioral data***
Retrieval trials that featured images from trials in which participants answered the attention check question incorrectly (*n* = `r nrow(cut_fix)` trials) were removed from analyses and *n* = `r length(beh_trial_counts$unique_trials)` retrieval trials were included for memory accuracy analyses. 